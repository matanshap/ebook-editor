...
def extract_text_from_pdf(path: Path) -> str:
    ...
    # Some real-world PDFs contain malformed escape sequences/streams.
    # Keep logs quiet and continue page-by-page in best-effort mode.
    ...
    for idx, page in enumerate(reader.pages, start=1):
        try:
            ...
            failed_pages += 1
            print(f"[warn] {path.name}: could not extract page {idx}: {exc}")
            pages.append("")

    if failed_pages == len(reader.pages) and len(reader.pages) > 0:
        raise RuntimeError(
            f"Failed to extract text from all pages in '{path}'. "
            "The PDF may be damaged or image-only and needs OCR/preprocessing."
        )
